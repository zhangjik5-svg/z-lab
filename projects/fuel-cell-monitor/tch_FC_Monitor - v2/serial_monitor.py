...
class DataRecorder:
    ...
    # Excel 中保存全部已解析字段。需要保留单位/格式的字段按原始文本保存，
    # 测量值和计数值则转换为数值，方便后续直接做公式、统计和绘图。
    FIELD_SPECS = [
        ("输出功率(W)", "POut", True),
        ("电池电压(V)", "UBat", True),
        ("输出电压(V)", "UOut", True),
        ...
        ("环境温度(℃)", "TAmb", True),
        ("电堆温度(℃)", "TStack", True),
        ("电堆运行时间", "StackOpTime", False),
        ("系统启动次数", "SystemStarts", True),
        ("电堆启动次数", "StackStarts", True),
        ("设备系统时间", "SystemTime", False),
    ]
    COLUMNS = ["序号", "电脑采集时间"] + [spec[0] for spec in FIELD_SPECS]
    FIELDS = [spec[1] for spec in FIELD_SPECS]
    SAVE_EVERY = 10  # 定期落盘；正常停止时还会再保存一次
    ...
    def _new_file(self):
        ...
        self.filepath = os.path.join(self.data_dir, base_name + ".xlsx")
        # 避免在同一秒内停止后重新开始时覆盖已有数据。
        suffix = 1
        while os.path.exists(self.filepath):
            self.filepath = os.path.join(
                self.data_dir, f"{base_name}_{suffix:02d}.xlsx"
            )
            suffix += 1
        self.wb = Workbook()
        ...
        self.ws.auto_filter.ref = f"A1:L1"
        self.ws.column_dimensions["A"].width = 9
        self.ws.column_dimensions["B"].width = 24
        for column in "CDEFGHIJKL":
            self.ws.column_dimensions[column].width = 18
        self.row_index = 0
    ...
    def add_row(self, parsed):
        ...
        # 丢弃不含任何目标字段的异常/空数据帧，避免产生整行空记录。
        if not any(field in parsed for field in self.FIELDS):
            return False
        ...
        self.row_index += 1
        row = [self.row_index, now]
        for _, field, numeric in self.FIELD_SPECS:
            raw = parsed.get(field)
            if numeric:
                value = _to_float(raw)
                row.append(value if value is not None else "")
            else:
                row.append("" if raw is None else str(raw))
        self.ws.append(row)
        # Excel 原生日期时间，包含日期和毫秒，便于跨文件合并和时间计算。
        self.ws.cell(row=self.ws.max_row, column=2).number_format = (
